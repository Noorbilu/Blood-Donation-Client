...
const Divider = () => {
  return (
    <div className="w-full bg-base-200 py-12">
      <div className="m-5 hidden md:block">
        <div className="border-t-2 border-dotted border-primary/40 mb-6"></div>
      </div>

      <h1 className="text-5xl font-bold text-primary text-center m-8 pb-4">
        Our Blood Donating Way
      </h1>
      ...
      <div className="max-w-6xl mx-auto px-4 space-y-10">
        {serviceData.map((item, index) => (
          <div
            key={index}
            className="flex flex-col md:flex-row items-center bg-base-100 rounded-2xl p-6 shadow-sm gap-6 border border-base-300"
          >
            <div className="w-full md:w-1/3 flex justify-center">
              <img
                src={item.img}
                alt={item.title}
                className="w-38 md:w-46 rounded-xl"
              />
            </div>

            <div className="hidden md:flex h-full justify-center">
              <div className="border-l-2 border-dotted border-primary/40 h-32"></div>
            </div>
